...
library(tidyverse)   # Datenaufbereitung, Joins, Pipelines, Visualisierung
library(readxl)      # Einlesen der Agglomerationsreferenz (Excel)
library(janitor)     # clean_names() zur Standardisierung der Spaltennamen
library(sf)          # Geodaten / Koordinatensystem EPSG:2056 (LV95)
...
#   - Stichprobengewicht auf das spaetere 3-Jahres-Pooling umskalieren.
...
hh_11_15_clean <- hh_11_15 |>
  ...
  mutate(
    ...
    # Gewicht auf 3-Jahres-Pooling umskalieren
    hh_weight_pool_3y = (hh_weight_2011_2015 * 5) / 3,
    # Datentypen setzen
...
setdiff(required_vars_hh, names(hh_11_15_clean))
# Befund: leer  ->  alle benoetigten Variablen vorhanden.

# Datentyp-Kontrolle.
glimpse(hh_11_15_clean)
...
na_analysis_by_year_11_15 <- hh_11_15_clean |>
  group_by(statyear) |>
  summarise(
    n_rows_year = n(),
    across(
...
hh_16_20_clean <- hh_16_20 |>
  ...
  mutate(
    ...
    hh_countof20_44   = hh_countof20_24 + hh_countof25_44,
    hh_countof65_plus = hh_countof65_79 + hh_countof80,
    # Gewicht auf 3-Jahres-Pooling umskalieren  (siehe PRUEFEN-Hinweis oben)
    hh_weight_pool_3y = (hh_weight_2016_2020 * 5) / 3,
    # Datentyp setzen
    statyear          = as.integer(statyear)
  ) |>
  # Auf den Untersuchungszeitraum eingrenzen (effektiv 2016-2017,
  # da der Datensatz erst ab 2016 beginnt)
  filter(statyear >= 2015 & statyear <= 2017) |>
  # Nur benoetigte Variablen behalten
  select(
    householdyearlyid,
    statyear,
...
# Vollstaendigkeitspruefung: Fehlt eine benoetigte Variable?
setdiff(required_vars_hh, names(hh_16_20_clean))
# Befund: leer  ->  alle benoetigten Variablen vorhanden.

# Datentyp-Kontrolle.
glimpse(hh_16_20_clean)
# Befund: alle Datentypen korrekt.
...
# NA-Analyse je Erhebungsjahr: Anteil fehlender Werte pro Variable und Jahr.
na_analysis_by_year_16_20 <- hh_16_20_clean |>
  group_by(statyear) |>
  summarise(
    n_rows_year = n(),
    across(
...
glimpse(hhm_11_15_clean)

hhm_11_15_clean <- hhm_11_15 |>
  mutate(
    across(
      c(
        hh_personnumber,
...
#   hh_11_15_clean, hh_16_20_clean      - bereinigte Haushaltsdaten
#   zp_11_15, zp_16_20                  - Zielpersonen (noch unbereinigt)
#   hhm_11_15, hhm_16_20                - Haushaltsmitglieder (noch unbereinigt)
